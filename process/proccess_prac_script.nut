...
::ShowHUDMessage <- function(message)
{
    local gt = Entities.FindByName(null, GAME_TEXT_ENT_NAME);
    if (gt == null)
    {
        printl("ShowHUDMessage: ERROR - No game_text named " + GAME_TEXT_ENT_NAME + ".");
        return;
    }

    EntFireByHandle(gt, "SetText", message, 0.0, null, null);
    EntFireByHandle(gt, "Display", "", 0.0, null, null);
}